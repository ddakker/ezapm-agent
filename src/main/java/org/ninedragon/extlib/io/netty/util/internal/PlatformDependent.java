 ...
 * under the License.
 */
package org.ninedragon.extlib.io.netty.util.internal;
...
import org.ninedragon.extlib.io.netty.util.CharsetUtil;
import org.ninedragon.extlib.io.netty.util.internal.chmv8.ConcurrentHashMapV8;
import org.ninedragon.extlib.io.netty.util.internal.chmv8.LongAdderV8;
 ...
 * the system property <strong>org.ninedragon.extlib.io.netty.noUnsafe</strong>.
 */
public final class PlatformDependent {
    ...
    private static final boolean DIRECT_BUFFER_PREFERRED =
            HAS_UNSAFE && !SystemPropertyUtil.getBoolean("org.ninedragon.extlib.io.netty.noPreferDirect", false);
    ...
    private static boolean hasUnsafe0() {
        boolean noUnsafe = SystemPropertyUtil.getBoolean("org.ninedragon.extlib.io.netty.noUnsafe", false);
        ...
        if (noUnsafe) {
            logger.debug("sun.misc.Unsafe: unavailable (org.ninedragon.extlib.io.netty.noUnsafe)");
            return false;
        }

        // Legacy properties
        boolean tryUnsafe;
        if (SystemPropertyUtil.contains("org.ninedragon.extlib.io.netty.tryUnsafe")) {
            tryUnsafe = SystemPropertyUtil.getBoolean("org.ninedragon.extlib.io.netty.tryUnsafe", true);
        } else {
            tryUnsafe = SystemPropertyUtil.getBoolean("org.jboss.netty.tryUnsafe", true);
        }

        if (!tryUnsafe) {
            logger.debug("sun.misc.Unsafe: unavailable (org.ninedragon.extlib.io.netty.tryUnsafe/org.jboss.netty.tryUnsafe)");
            return false;
        }
    ...
    private static boolean hasJavassist0() {
        ...
        boolean noJavassist = SystemPropertyUtil.getBoolean("org.ninedragon.extlib.io.netty.noJavassist", false);
        logger.debug("-Dio.netty.noJavassist: {}", noJavassist);

        if (noJavassist) {
            logger.debug("Javassist: unavailable (org.ninedragon.extlib.io.netty.noJavassist)");
            return false;
        }
    ...
    private static File tmpdir0() {
        File f;
        try {
            f = toDirectory(SystemPropertyUtil.get("org.ninedragon.extlib.io.netty.tmpdir"));
            if (f != null) {
    ...
    private static int bitMode0() {
        ...
        int bitMode = SystemPropertyUtil.getInt("org.ninedragon.extlib.io.netty.bitMode", 0);
        if (bitMode > 0) {
